...
cd "$(dirname "$0")/.."
```
**Issue**: SC2164 - Command failure not handled. If cd fails, script continues in wrong directory.
**Fix**: 
```bash
cd "$(dirname "$0")/.." || exit 1
```
...
```
**Issue**: SC2155 - Declaring and assigning in one line masks the return value. If both stat commands fail, the error is hidden.
**Fix**:
```bash
local file_size
file_size=$(stat -f%z "build/web/data.csv" 2>/dev/null || stat -c%s "build/web/data.csv" 2>/dev/null)
```
**Note**: Fallback pattern for macOS (`-f%z`) and Linux (`-c%s`) is already correct.
