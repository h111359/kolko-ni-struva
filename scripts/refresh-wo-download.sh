#!/bin/bash
###############################################################################
# refresh-wo-download.sh - Data Refresh Script (Without Download)
# 
# Purpose: Process existing raw data and generate a static site with the last 
#          2 days in /build/web (skips download step)
#
# Usage:
#   ./scripts/refresh-wo-download.sh              # Process last 2 days
#   ./scripts/refresh-wo-download.sh --help       # Show usage information
#
# Features:
#   - Processes existing raw data (no download)
...
#
# Examples:
#   # Standard refresh (process last 2 days, no download)
#   bash scripts/refresh-wo-download.sh
#
#   # Run from project root
#   cd /path/to/kolko-ni-struva && bash scripts/refresh-wo-download.sh
#
###############################################################################
...
# Configuration
LOG_FILE="logs/refresh_wo_download_$(date +%Y%m%d_%H%M%S).log"
...
Data refresh script for Kolko Ni Struva project (without download step).
Processes existing raw data and generates a static site with the last 2 days.
...
LOGS:
    All operations are logged to: logs/refresh_wo_download_YYYYMMDD_HHMMSS.log
...
get_dates() {
    # Calculate dates for the last 2 days
    TODAY=$(date +%Y-%m-%d)
    YESTERDAY=$(date -d "yesterday" +%Y-%m-%d)
    
    log_info "Target dates for processing:"
...
check_existing_data() {
    local dates=("$@")
    log_info "Checking existing raw data..."
    ...
    if [ ${#missing_dates[@]} -gt 0 ]; then
        log_warning "⚠️  WARNING: Some days have missing data:"
        for missing_date in "${missing_dates[@]}"; do
            log_warning "   - $missing_date"
        done
        log_warning "These days will be skipped in processing"
    fi
    
    if [ $found_count -eq 0 ]; then
        log_error "No data files found for any of the target dates"
        log_error "Please run the download step first or use refresh.sh"
        return 3
    fi
    
    log_success "Found data for $found_count out of ${#dates[@]} days"
    return 0
}

process_data() {
    local dates=("$@")
    log_info "🔄 Starting data normalization and processing..."
...
main() {
    ...
    echo "╔════════════════════════════════════════════════════════════════╗"
    echo "║    Kolko Ni Struva - Data Refresh (Without Download)          ║"
    echo "╚════════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    # Step 4: Get target dates (last 2 days)
    get_dates
    
    # Step 5: Check existing data
    check_existing_data "$YESTERDAY" "$TODAY" || exit 3
    
    # Step 6: Process data for last 2 days
