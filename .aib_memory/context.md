...
- **`react-app/src/lib/supabase.js`:** Supabase client singleton using `import.meta.env.VITE_SUPABASE_URL` and `import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY` (loaded by Vite from project-root `.env` file). No hardcoded credentials. Validates the key before instantiation: rejects keys starting with `sb_secret_` or matching the 3-segment JWT format by setting `credentialsError` with a descriptive Bulgarian message; no Supabase client is created when `credentialsError` is non-null.
...
| `VITE_SUPABASE_PUBLISHABLE_KEY` | `.env` (project root) | (none; required) | Operator |
...
- `.env` (project-root): contains all six environment keys: three server-side secrets (`DATABASE_URL`, `NETLIFY_AUTH_TOKEN`, `NETLIFY_SITE_ID`) plus `SUPABASE_SECRET_KEY` (documented placeholder for future server-side Supabase REST API use; not currently wired to any Python module) and two client-safe keys (`VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`); excluded from VCS by `.gitignore`. `.env.example` committed (placeholder only, single consolidated template). No secret key in the React app.
...
Six environment keys are consolidated into a single `.env` file at project root: (1) `DATABASE_URL` for Python ETL sync to Supabase; (2) `NETLIFY_AUTH_TOKEN` and `NETLIFY_SITE_ID` for Netlify deployment; (3) `SUPABASE_SECRET_KEY` as a server-side placeholder for future Supabase REST API use (not currently consumed); (4) `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY` for React frontend via Vite build (VITE_-prefixed variables are client-exposed). The single root `.env` serves both Python scripts and React build. In Netlify production, inject via Netlify site environment variables (not committed `.env` files).
...
Netlify deployment: set base directory to `react-app`; `netlify.toml` configures `build.command = "npm run build"` and `publish = "dist"`. Inject `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY` as Netlify site environment variables.
...
2. `cd react-app && cp .env.example .env` — fill in `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY`.
3. `npm install`.
